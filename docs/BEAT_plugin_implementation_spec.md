...
Hotbar は個人 CP を slot 0、飛行切替を slot 1、位置調整用の盾を slot 2 とする。
...
高難易度の練習中と本番中は、参加者へ位置調整用の盾を配布する。

- Hotbar の slot 2へ配置
- 盾を構えた際の標準的な移動速度低下を利用する
- 独自の右クリック処理は行わない
- PDCでBEAT専用アイテムとして識別し、重複配布しない
- 練習中または本番中の再接続時は必要に応じて再配布する
- 準備フェーズへの移行、脱落、全Course完走、競技終了、中止、リセット時に回収する
- Adminのみのプレイヤーには配布せず、参加者として登録されている場合に配布する
